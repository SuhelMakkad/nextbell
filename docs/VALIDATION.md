...
- Android debug APK and unsigned release AAB build with the new Firebase/Credential Manager dependencies and updated Gradle locks. Locked Flutter-CLI builds initially exposed missing Kotlin common and native sign-in runtime entries. Regenerating the app and plugin runtime configurations fixed this; both `flutter build apk --debug` and `flutter build appbundle --release` now pass without updating locks during the build. These unconfigured validation artifacts are not the installable hosted-beta build; generate native Firebase resources and compile with `cloud.local.json` after hosted setup.
