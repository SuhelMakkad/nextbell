...
7. Verify domain ownership in Google Search Console with the Google account associated with project `nextbell-507711`. Add `nextbell.org` to Google's authorised domains and enter the homepage, privacy, and terms URLs. The native app audience is now External / In production, and branding is verified and published. Calendar/Tasks sensitive-scope verification remains outstanding; this is not a web OAuth client or backend deployment.
